...
Contrail is a local companion for IVAO Altitude. It places a local proxy between
...
architecture. In Remote Preview, the user's PC still runs the local Contrail
...
Contrail has four practical goals:
...
Altitude and PilotCore for the actual IVAO session. Contrail acts as a local
...
- **Local agent**: the Contrail proxy running on the same PC as Altitude.
...
Contrail's local mode is the safest default. Everything runs on the user's PC or
local network, and the browser connects to the local webapp served by the
proxy.

```mermaid
flowchart TD
  PilotUI["Altitude PilotUI"] -->|"4827 simulator/core"| Agent["Contrail local agent"]
...
1. PilotUI is configured to use the local IPv4 address printed by Contrail as
   its simulator address.
2. PilotUI connects to Contrail instead of directly to PilotCore.
3. Contrail forwards PilotUI/PilotCore traffic so the normal Altitude flow
   remains intact.
4. When PilotCore receives IVAO server information, Contrail rewrites the
   relevant endpoints so FSD and TS2 traffic pass through the local proxy.
5. Contrail parses only the traffic needed for the webapp and forwards the
...
### 4.2 `webapp/index.html`

The webapp is a single static HTML/CSS/JavaScript application. In local mode it
connects to the local agent through `/ws`. In Remote Preview mode it connects
to a relay URL configured through query parameters or `Settings > Remote`.
...
PilotUI normally talks to PilotCore on a simulator/core port. Contrail listens
...
When PilotCore receives an IVAO FSD endpoint, Contrail rewrites the endpoint to
`127.0.0.1`. PilotCore then connects to Contrail's local FSD proxy, and Contrail
connects onward to the real IVAO FSD server.

Contrail parses FSD traffic for:
...
When IVAO announces a TS2 voice server, Contrail rewrites the voice server
address to the local network IP selected at startup. PilotCore then sends TS2
voice traffic through Contrail.

Contrail forwards TS2 traffic to the real server and inspects the packet class
needed for voice receive and transmit session caching.

## 6. Radio and Station Model

Contrail maintains current COM1 and COM2 frequencies from multiple sources:
...
```mermaid
flowchart LR
  TS2["TS2 server"] -->|"voice packet"| Agent["Contrail agent"]
...
```mermaid
flowchart LR
  Mic["Browser microphone"] -->|"PCM16 + CTX1 prefix"| Agent["Contrail agent"]
...
libspeex through ffmpeg. The encoder produces Ogg/Speex output. Contrail unwraps
...
Current limitation: after a proxy restart, Web TX still needs one real Altitude
PTT press so the agent can cache the TS2 transmit session seed. Removing this
requirement is planned, but it is intentionally deferred while the remote
foundation is being stabilized.
...
```mermaid
flowchart TD
  Phone["Remote browser or phone"] <-->|"HTTPS/WSS"| Relay["Contrail Relay"]
  Agent["Contrail local agent"] -->|"outbound WSS"| Relay
...
1. The user starts Contrail on the Altitude PC.
2. The local agent connects outbound to the relay with `source=agent`.
...
the simpler JSON store. The local webapp can request a fresh short-lived code
without restarting the agent; this sends another `pairing.begin` message and
replaces any still-open code for the same agent.
Production remote access should add real browser accounts, persistent sessions,
full device lifecycle management, and broader login/session audit logging.
...
Contrail uses defense in depth:
...
Contrail handles data that can be sensitive:
...
- The relay should not store chat or audio.
...
- Web TX still needs one real Altitude PTT press after proxy restart.
- Web TX may have slight crackle compared with native Altitude TX.
- IVAO does not echo the user's own TX audio.
- Remote TX is preview-routed and has successful live IVAO validation, but still
  needs broader validation across more networks and listeners.
- Remote Preview does not yet have production accounts.
- Pairing persistence is file-based preview storage.
- The relay is not a complete hosted account system.
- The webapp has a desktop-first layout; a dedicated mobile UI is planned.
...
- Dedicated mobile UI for phones and tablets.
- Renewable pairing codes without restarting the local proxy.
- Clearer paired-browser management and revocation UI.
- Production authentication and account/device records for hosted remote access.
...
- Removing the first real PTT requirement for Web TX.
...
Contrail is a local-first bridge that turns Altitude traffic into a browser
...
The result is a practical experimental architecture: local control works today,
remote RX works through a self-hosted relay, remote TX routing exists in
preview, and the remaining work is mostly about hardening, live validation,
mobile ergonomics, and production-grade remote identity.
