# Contrail Remote Threat Model

Status: planned design. This document describes the intended remote architecture, not the current local-only release.
...
- Which remote audio design to use before implementing remote RX/TX: browser-native codecs, WASM Speex, or agent-side transcoding.
- Whether to implement end-to-end payload encryption before or after the first relay prototype.
- Which authentication method should be mandatory for the official relay.
- Whether the official relay should allow open registration or invite-only beta.
