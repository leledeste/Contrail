...
Contrail should stay easy to audit and redistribute. New dependencies are
allowed only when they clearly reduce project risk or complexity, and their
licenses must be compatible with Contrail's MIT license.
...
- Avoid dependencies that force Contrail itself to change license.
...
users who self-host, redistribute, or inspect Contrail.
...
Contrail currently calls `ffmpeg` from the system `PATH` and does not bundle
...
SQLite is acceptable as the default self-host database because Contrail's
...
If Contrail later serves a large public relay, PostgreSQL can be added as an
