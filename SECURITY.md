...
This document covers the Contrail local agent, webapp, planned relay, and self-hosting configuration.

Contrail is experimental software. It interacts with IVAO Altitude traffic and browser microphone access, so remote access must be designed defensively.
...
## Planned Remote Security Controls
...
Production authentication, per-agent tokens, browser sessions, and Shared
Cockpit permission boundaries are planned in
