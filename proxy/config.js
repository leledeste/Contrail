...
function validateConfig(config, defaults) {
  ...
  // starting. Instead, Contrail warns and falls back to the known-safe default.
  const schemaKeys = new Set(Object.keys(defaults));
  for (const key of Object.keys(config)) {
    if (!schemaKeys.has(key)) console.warn(`[CONFIG] ${key}: unknown option ignored by Contrail`);
  }
