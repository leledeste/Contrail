...
function createRemoteAgent(options) {
  ...
  const deviceId = normalizeRemoteDeviceId(config.remoteDeviceId)
    || normalizeRemoteDeviceId(`contrail-${options.hostname || 'local'}`);
  const deviceName = config.remoteDeviceName || `${options.hostname || 'Local'} Contrail`;
  ...
  function connect() {
    ...
    const next = new WebSocket(relayWsUrl, { perMessageDeflate: false });
    ws = next;

    next.on('open', () => {
      logger.log(`[REMOTE] Agent connected as ${deviceName}`);
      send(MESSAGE_TYPES.AGENT_HELLO, { deviceId, deviceName }, 'agent-hello');
      send(MESSAGE_TYPES.PAIRING_BEGIN, { deviceName }, 'pairing-begin');
      sendStatus();
      sendRadioState();
      sendStationsState();
    });
  ...
  function renewPairingCode(report = () => {}) {
    ...
    const ok = send(MESSAGE_TYPES.PAIRING_BEGIN, { deviceName }, `pairing-renew-${Date.now().toString(36)}`);
...
function buildRemoteRelayWsUrl(config, logger = console) {
  ...
  try {
    ...
    url.searchParams.set('token', config.remoteRelayToken);
    return url.toString();
  } catch (err) {
