...
function createFsdProxy(options) {
  ...
  // traffic also passes through Contrail.
  ...
  function handleFsdLine(line, direction) {
    ...
    if (msg.kind === 'flight_plan_status') {
      state.updateFlightPlanStatus(msg.status);
      return;
    }
  ...
  function sendWeatherRequest(kind, icao, reportError = () => {}) {
    ...
    return writeFsdLine(`&D${cs}:SERVER:${type}:${code}`, reportError);
  }
