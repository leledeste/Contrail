const XPDR_STORAGE_KEY = 'contrail.xpdrState.v1';
const REMOTE_STORAGE_KEY = 'contrail.remoteSettings.v1';
const REMOTE_BROWSER_ID_STORAGE_KEY = 'contrail.remoteBrowserId.v1';
const ACCOUNT_STORAGE_KEY = 'contrail.accountSettings.v1';
const AUTH_MANUAL_STORAGE_KEY = 'contrail.authManualMode.v1';
...
function defaultRemoteRelay() {
  // Hosted self-installs commonly use app.example.com and relay.example.com.
...
function remoteRelayDraft() {
  ...
  // app.example.com -> relay.example.com convention.
...
async function refreshAccountStatus() {
  ...
  try {
    const body = await accountRequest('/account/api/me');
    state.account = {
      authenticated: true,
      userId: body.user?.userId || '',
      userName: body.user?.userName || body.user?.userId || '',
      registrationEnabled: state.account.registrationEnabled,
    };
...
async function runRemotePreflight() {
  ...
  try {
    ...
    if (!health.ok || health.service !== 'contrail-relay') {
...
function remoteSummary() {
  ...
  if (!state.remoteIdentity?.userId) return { text: 'Relay connected, waiting for identity', tone: 'warning' };
  if (!state.remoteDevices.size) return { text: state.account.authenticated ? 'Logged in, no agent online yet' : 'Token valid, no agent online yet', tone: 'warning' };
  if (!state.remoteSelectedDeviceId) return { text: state.account.authenticated ? 'Agent online, selecting device' : 'Agent online, browser not paired', tone: 'warning' };
...
function remoteAgentStatusText() {
  ...
  if (!state.remoteDevices.size) return state.account.authenticated ? 'No agent online' : 'No paired agent';
  if (!state.remoteSelectedDeviceId) return state.account.authenticated ? 'Agent not selected' : 'Agent not paired';
...
function remotePairingStatusText() {
  ...
  if (!state.remote.enabled) return 'Local mode';
  if (state.remoteSelectedDeviceId && state.remoteDevices.has(state.remoteSelectedDeviceId)) return 'Paired and online';
  if (state.remoteSelectedDeviceId) return 'Paired, agent offline';
  if (state.account.authenticated) return state.remoteDevices.size ? 'Account access' : 'Logged in';
...
function accountErrorMessage(err, action) {
  ...
  switch (err?.code) {
    case 'account_mode_unavailable':
      return 'Hosted accounts are not enabled on this relay. Use Manual Relay Setup or enable SQLite auth.';
...
async function rotateAccountAgentToken() {
  try {
    ...
    addLocal('Agent token rotated. Update config.json and restart Contrail on the Altitude PC.');
  } catch (err) {
...
function readAccountForm(includeDisplayName) {
  ...
  if (!/^[a-z0-9._-]{2,48}$/.test(userId)) {
    ...
    addErrorMessage('Use a 2-48 character account username: letters, numbers, dot, underscore, or dash.');
    return null;
  }
  if (password.length < 10) {
    setAuthStatus('Password must be at least 10 characters.', 'bad');
    addErrorMessage('Account password must be at least 10 characters.');
    return null;
  }
...
function showAgentToken(token) {
  const text = `Agent token shown once:\n${token}\n\nPut it in config.json as remoteRelayToken, then restart Contrail on the Altitude PC.`;
...
// Command suggestions are the same commands accepted by runCommand().
// Keeping this table separate makes autocomplete UI independent from
// command execution.
const COMMAND_SUGGESTIONS = [
  { name: '.metar', args: 'ICAO', description: 'Request METAR' },
  { name: '.wx', args: 'ICAO', description: 'Request METAR' },
  { name: '.taf', args: 'ICAO', description: 'Request TAF' },
  { name: '.atis', args: 'CALLSIGN', description: 'Request ATIS' },
  { name: '.msg', args: 'CALLSIGN text', description: 'Send private message' },
  { name: '.m', args: 'CALLSIGN text', description: 'Send private message' },
  ...
  { name: '.sq', args: 'code', description: 'Set squawk' },
  { name: '.xpdr', args: '', description: 'Toggle transponder' },
  { name: '.xp', args: '', description: 'Toggle transponder' },
  { name: '.ident', args: '', description: 'Send IDENT' },
  { name: '.id', args: '', description: 'Send IDENT' },
];
...
function showRemoteSetupPrompt() {
  ...
  const missing = !state.remote.relay ? 'relay URL and token' : 'relay token';
  setOffline('Remote setup required', `Enter the ${missing} in Settings > Remote, then apply and pair this browser.`);
  setRemoteCheck(!state.remote.relay ? 'Missing relay URL' : 'Missing relay token');
  showRemoteSettingsAction();
...
function showAuthGate(message = '') {
  ...
  else setAuthStatus('Use the same account on every browser you want to connect.');
...
function remoteCloseReason(event) {
  ...
  if (reason) {
    return {
      ...
      detail: reason,
      overlayText: `The relay closed the connection: ${reason}. Open Remote Settings and check the relay token and pairing code.`,
    };
  }
  if (code === 1006) {
    return {
      ...
      overlayText: 'The relay connection could not be opened. Open Remote Settings and check the relay URL/token, then apply and pair this browser.',
    };
  }
  return {
    status: 'Relay disconnected',
    detail: code ? `WebSocket closed with code ${code}.` : '',
    overlayText: 'Waiting for the relay connection to recover. If this persists, open Remote Settings and check the relay URL/token and pairing code.',
  };
}
...
function renewRemotePairingCode() {
  ...
  if (state.remote.enabled) {
    addErrorMessage('Open the local Contrail webapp on the Altitude PC to renew the pairing code.');
    return false;
  }
  if (!state.ws || state.ws.readyState !== WebSocket.OPEN) {
...
function handleMessage(data) {
  switch (data.kind) {
    case 'init':
      ...
      setFlightPlanStatus(data.flightPlanStatus || 'missing');
    ...
    case 'flight_plan_status':
      setFlightPlanStatus(data.status);
      break;
    case 'remote_pairing':
...
function applyRemotePairingRevoked(payload) {
  ...
  setOffline('Remote agent unpaired', 'Enter a new pairing code to control this agent again.');
  showRemoteSettingsAction();
...
function updateRemoteDevice(device) {
  ...
  if (device.online && (!state.remoteSelectedDeviceId || !selectedOnline)) {
    ...
    setOffline('Remote agent offline', 'Start Contrail on the PC with Altitude and keep remote agent mode enabled.');
    showRemoteSettingsAction();
  }
  updateSettingsView();
}
...
function setOverlay(mode) {
  ...
  if (mode === 'altitude') {
    const title = state.remote.enabled ? 'Remote agent waiting' : 'Altitude disconnected';
    const text = state.remote.enabled
      ? 'The relay is connected, but the local agent is not connected to IVAO yet.'
    ...
    setOffline(
      ...
      state.remote.enabled ? 'Check the relay URL, token, and network connection.' : 'Start start.bat or node proxy.js.'
    );
...
function setFlightPlanStatus(status) {
  // Flight plan status comes from FSD traffic. Repeated server errors are
  // reduced to this one pill so the chat stays readable.
  ...
  $('flight-plan-text').textContent = filed ? 'Flight plan' : 'No flight plan';
  updateSettingsView();
}
...
function updateSettingsView() {
  ...
  setText('settings-flight-plan', state.flightPlanStatus === 'filed' ? 'Flight plan' : 'No flight plan');
  ...
  $('remote-mode').textContent = state.remote.enabled ? 'Remote preview' : 'Local';
  setText('settings-remote-mode', state.remote.enabled ? 'Remote preview' : 'Local');
  ...
  renewButton.title = state.remote.enabled
    ? 'Pairing codes are generated by the local Contrail webapp on the Altitude PC.'
...
function renderStationMenus() {
  ...
  const list = [
    UNICOM_STATION,
    ...getSortedStations().filter(station => station.freq && station.callsign !== UNICOM_STATION.callsign),
  ];
...
function stationForFrequency(freq) {
  ...
  const match = getSortedStations().find(station => normalizeFreq(station.freq) === value);
...
function bindHoldButton(id, onStart) {
  ...
  const begin = event => {
    ...
    activePointerId = event.pointerId ?? 'legacy';
