...
const INTERNAL_PORT = 4827;              // PilotUI -> Contrail -> PilotCore
...
function updateVoiceServer(server) {
  ...
  if (after !== before) clearWebTxReadiness('voice server changed');
  return after;
}
...
pilotBridge.server.on('error', failListen(
  ...
  LAN_IP,
  INTERNAL_PORT,
  'Close Altitude/PilotCore if it is already running, start Contrail first, then reconnect Altitude.'
));
...
console.log('');
console.log('=======================================================');
console.log('  Contrail');
console.log('=======================================================');
console.log('  1. Start Contrail before connecting Altitude/PilotUI');
