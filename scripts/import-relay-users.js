...
// CONTRAIL_RELAY_AUTH_MODE=sqlite-fallback to test SQLite tokens with .env
...
const USERS_KEY = 'CONTRAIL_RELAY_USERS';
const LEGACY_TOKEN_KEY = 'CONTRAIL_RELAY_TOKEN';
...
function main(argv) {
  ...
  try {
    ...
    console.log('[relay-db] Import complete. Set CONTRAIL_RELAY_AUTH_MODE=sqlite-fallback to test DB auth with .env fallback.');
  } finally {
    db.close();
  }
}
...
function readRelayUsers(env) {
  ...
  const legacyToken = String(env[LEGACY_TOKEN_KEY] || '').trim();
  if (legacyToken) {
    if (users.some((user) => user.userId === 'default')) {
      fail(`${USERS_KEY} cannot include reserved user "default" when ${LEGACY_TOKEN_KEY} is set.`);
    }
    users.push({
      userId: 'default',
      displayName: 'Default',
      token: legacyToken,
      tokenName: 'Legacy relay token',
    });
  }
  return users;
}
...
function printHelp() {
  console.log(`Contrail relay .env to SQLite importer

Usage:
  npm run relay:db:import-users -- [--env path/to/.env] [--db path/to/contrail.db] [--list]

Examples:
  npm run relay:db:import-users -- --env apps/relay/.env --db .contrail-relay/contrail.db --list
  npm run relay:db:import-users -- --env infra/docker/.env --db ./contrail.db
`);
}
