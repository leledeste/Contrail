...
// one Contrail agent and one remote browser. It verifies the security boundary
...
async function main() {
  ...
  relayDataDir = fs.mkdtempSync(path.join(os.tmpdir(), 'contrail-remote-test-'));
  ...
  try {
    await agent.sendAndWait(
      ...
      { deviceId: agentDeviceId, deviceName: 'Test Agent' },
...
async function startRelay(port, dataDir) {
  relayProcess = spawn(process.execPath, [path.join(root, 'apps/relay/index.js')], {
    ...
    env: {
      ...
      CONTRAIL_RELAY_PORT: String(port),
      CONTRAIL_RELAY_USERS: `primary=${token},other=${otherToken}`,
      ...
      CONTRAIL_RELAY_DATA_DIR: dataDir,
      CONTRAIL_RELAY_PAIRINGS_FILE: path.join(dataDir, 'pairings.json'),
    },
...
async function checkHealth(baseHttp) {
  ...
  assert.strictEqual(health.service, 'contrail-relay', 'health service should identify the relay');
...
async function connectClient(baseWs, source, options) {
  ...
  const ws = new WebSocket(`${baseWs}?${params.toString()}`, { origin: options.origin });
...
async function assertBrowserCommandsReachAgent(browser, agent) {
  const commands = [
    ...
    [MESSAGE_TYPES.WEATHER_REQUEST, { kind: 'metar', icao: 'LIMC' }],
