...
async function assertEnvOnlyModeIgnoresDatabase() {
  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'contrail-relay-env-auth-'));
  let relay = null;
...
async function assertDbAuthWithEnvFallback() {
  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'contrail-relay-db-auth-'));
  let relay = null;
...
async function assertDbAuthStrictMode() {
  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'contrail-relay-db-auth-sqlite-'));
  let relay = null;
