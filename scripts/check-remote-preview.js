...
const relayEnvPath = process.env.CONTRAIL_RELAY_ENV_FILE || path.join(root, 'apps', 'relay', '.env');
...
function readEnvFile(filePath) {
  ...
  for (const line of text.split(/\r?\n/)) {
    ...
    if (/^[A-Z0-9_]+$/.test(key)) env[key] = value;
  }
  return env;
}
...
async function main() {
  ...
  const relayHost = relayEnv.CONTRAIL_RELAY_HOST || '127.0.0.1';
  const relayPort = Number(relayEnv.CONTRAIL_RELAY_PORT || 8787);
  const relayToken = relayEnv.CONTRAIL_RELAY_TOKEN || '';
  ...
  else warn('relay token', 'set CONTRAIL_RELAY_TOKEN to a non-example value before serious testing');
